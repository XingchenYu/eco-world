...
- `tick`：模拟步，生态系统前进一次的最小更新时间单位。
- `habitat`：栖息地，指河道、浅湖、森林、湿地等环境类型。
- `patch`：局部资源斑块，通常指某个位置上的资源点。
- `microhabitat patch`：微栖位资源点，指树冠位、夜栖位、花蜜位、岸栖位等。
- `prey`：猎物。
- `predator`：捕食者。
- `soft carrying capacity`：软承载抑制，意思是种群不会被硬性截断，而是逐渐受到压力限制。
- `habitat fast filter`：栖息地快速粗筛，先快速排除明显不合适的位置。
