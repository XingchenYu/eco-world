...
- `patch`：资源斑块，指某个局部区域里的资源点或栖位点。
- `microhabitat`：微栖位，指树冠位、夜栖位、花蜜位、岸栖位等小尺度栖息资源。
- `habitat`：栖息地，指物种适合生存的环境类型，比如森林、湿地、浅湖、河道。
- `prey`：猎物，也就是被捕食的一方。
- `predator`：捕食者，也就是主动捕食其他生物的一方。
- `actor`：生态驱动因子，这里指汇总出来的关键生态状态指标。
- `soft carrying capacity`：软承载抑制，不是硬上限，而是种群过高后通过繁殖下降、死亡上升等方式被逐渐压制。
- `night_swarm`：夜间飞虫群资源，主要服务夜行链。
- `shore_hatch`：岸带羽化资源，主要服务湿地链和近岸觅食链。
- `canopy_forage`：树冠持续觅食资源，主要服务树冠活动物种。
...
- 基础中层：`insect`、`night_moth`、`rabbit`、`mouse`、`deer`
...
- 大地图高种群默认配置
- 空间索引和多层缓存优化
- ecosystem actor 已改成 tick 级缓存，减少主循环和统计阶段的重复计算
- 夜间飞虫与岸带羽化资源已接入夜行链和湿地链
