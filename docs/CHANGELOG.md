...
### v4.0-alpha154 (2026-04-26)

- ✅ 根据实际运行反馈补强玩法理解：
  - 世界图默认隐藏手动 `调查线/修复线/通道线` 和 `推进模拟` 入口，只保留主线推荐区、开始主线、回灌报告三类动作。
  - 点击 `开始主线` 会自动进入后端推荐的主线区域，不再要求玩家先手动切到正确区域。
  - 世界图 HUD 改成 `本轮玩法` 与 1-5 步操作链，减少后台术语和卡片选项对玩法理解的干扰。
  - 区域进入后增加主线开场任务卡，显示章节名、本轮玩法、目标说明和 3 步完成路径。
  - 回灌报告后增加世界图主线结算卡，突出章节推进、后端写回结果和下一轮主线目标。
  - 世界图 HUD 和主线结算卡增加主线进度条，直接展示后端生态系统计算出的章节完成度。
  - 世界图 HUD 和主线结算卡增加章节路线图：建档 -> 修复 -> 通道 -> 扩展 -> 复苏。
  - 主线章节路线改为由后端 `gameplay_state.mainline.chapter_route` 导出，前端不再硬编码章节状态。
  - 后端章节路线节点增加 `goal`，世界图当前章节会显示该章解决的生态问题。
  - 后端主线增加 `chapter_payoff`，世界图会解释当前章节对生态系统的实际收益。
  - `开始主线` 会把章节目标和生态收益传入区域探索，区域开场任务卡同步显示“生态意义”。
  - 撤离报告和后端 applied report 会保留主线章节目标/收益，便于追溯每轮回灌贡献。
  - 待回灌 HUD 和回灌结算卡会显示本轮主线贡献与生态收益，让回灌结果更像任务结算。
  - 后端世界公告会把撤离摘要、主线贡献和生态收益合并成一条回灌公告，便于后续做编年史。
  - 世界图 HUD 会直接显示最新后端世界公告，让主线回灌编年史在默认界面可见。
  - 世界图 HUD 增加最近三条公告时间线，并兼容旧字符串公告与新结构化公告。
  - 世界地图支持鼠标滚轮缩放、右键拖动视野，点击区域标记仍用于切换区域。
  - 手动行动线仍保留为内部调试路径，但默认不再打断主线入口。
  - 区域动物活动半径扩大，行为不再只围绕单个小热点打转。
  - 区域探索顶部 HUD 增加本区地貌和代表物种，让森林、草原、湿地、海岸进入后更容易区分。
  - 区域探索地图从 `2800x1700` 扩展到 `3600x2300`，热点、出口、障碍和出生点按比例迁移，避免内容仍挤在旧尺寸里。
  - 动物靠近玩家时不再被脚本推开或快速逃离，记录判定范围放大，避免“刚接近就消失”的体验。
  - 掠食者追猎改成更慢的观察事件：猎食者移动速度下降，命中需要更近距离和更长追踪时间。
  - 动物绘制增加物种差异和常驻名称条，大象、长颈鹿、羚羊、掠食者的轮廓和尺寸不再完全一样。
  - 区域地图再次扩展到 `4800x3200`，四类地貌贴图重生成，避免大世界继续露出旧小图边界。
  - 区域探索接入项目自生成 PNG 动物资源，优先绘制狮子、羚羊、象、长颈鹿、斑马、犬科、鳄鱼、秃鹫、鱼类贴图。
  - 玩家进入动物观察范围后，动物会进入 `观察` 停留状态，不再一边靠近一边继续巡游出屏幕。
  - 动物靠近判定提前到行为更新前执行，并增加 `observe_lock_until` 观察锁，靠近后会冻结几秒。
  - 当前偶遇动物会在玩家绘制之后再重绘一次高亮版本，避免被玩家图标或近距离重叠遮住，看起来像“消失”。
  - 观察动物改为在所有 UI 之后最终绘制，并取消近距离重定位，只在原地冻结，避免被底部卡片或玩家遮挡后误判为消失。
  - 当前观察目标增加独立 `active_observation_species_id` 锁，即使偶遇判定短暂丢失也会继续高亮显示 4 秒。
  - 靠近动物的观察锁提升到偶遇判定层，320px 内立即锁定，不再依赖动物行为循环后置触发。
  - 新增屏幕固定“观察镜”：锁定动物会在屏幕中央上方显示贴图、距离、锁定剩余时间和屏幕坐标，便于确认它没有从数据层消失。
- ✅ 区域探索主线玩法改成默认可读的四步流程：
  - 画面上方常驻 `主线目标`，直接告诉玩家下一步该去哪、按什么键、为什么做。
  - 右下角默认显示 `记录动物 -> 采样热点 -> 达到情报阈值 -> 撤离回灌`，不再让玩家先理解内部系统术语。
  - `修复` 和 `通道` 行动进入区域后会显示各自的主线目标，不再只靠世界图按钮猜含义。
  - `godot/data/selected_expedition_region.json` 与 `world_strategy_intent.json` 已加入忽略，避免把临时运行态提交进仓库。
- ✅ 世界图主线入口继续降噪：
  - 主按钮改为 `开始主线`。
  - 行动按钮改为 `调查线/修复线/通道线`，表达它们是三种玩法路线，而不是抽象后台指令。
  - 世界图 HUD 直接显示 `进入后做什么` 与 `开始主线 -> WASD -> Space -> E -> 回灌报告` 的完整操作链。
- ✅ 区域出口闭环修正：
  - 旧逻辑会在部分出口按 `E` 后直接切到下一片区域，和“撤离回世界图、回灌报告”的主线闭环冲突。
  - 现在所有出口统一为：按 `E` 生成撤离报告 -> 回世界图 -> 点击 `回灌报告` 写回后端生态系统。
  - 通道线仍会把目标区域写进报告，用于后端强化区域连接，但不会跳过回灌步骤。
- ✅ 待回灌状态改成独立结算态：
  - 不再继续显示普通 `开始主线` 流程，避免玩家误以为可以直接开下一轮。
  - HUD 会显示本轮成果：行动线、任务是否完成、情报总量、动物/热点情报、主情报方向、档案等级。
  - 通道线会额外显示目标连接区域，明确回灌后强化哪条区域通道。
  - 主按钮 `回灌报告` 加高，成为待回灌状态下唯一主线动作。
- ✅ 回灌完成后也会显示具体后端反馈：
  - 调查线会显示调查覆盖与资源判断被补强。
  - 修复线会显示被压低的风险项与韧性收益。
  - 通道线会显示目标区域连接是否成功增强。
  - 玩家能看到“报告待回灌 -> 回灌完成 -> 世界状态已吸收”的完整闭环。
- ✅ 区域调查交互增加目标锁定：
  - 开始按住 `Space` 记录动物或采样热点后，会锁定当前目标，动物轻微移动不会立刻打断进度。
  - 锁定范围放宽到 260，只有松开 `Space` 或离目标太远才会掉进度。
  - HUD 会显示 `已锁定记录/采样`，减少“动物靠近后消失、记录断掉”的误解。
- ✅ 锁定调查增加现场可视化：
  - 被锁定的动物/热点周围会出现亮色调查环。
  - 调查环会按 `Space` 按住时间显示进度弧。
  - 目标旁边直接显示 `锁定记录/锁定采样` 和进度百分比，减少目标丢失感。
- ✅ 区域动物行为从匀速绕圈改为活动状态驱动：
  - 动物会在 `移动/停留/觅食/警觉/巡猎/潜伏/取水/盘旋` 等状态间切换。
  - 不同状态会改变移动速度、停留倾向、靠近水源或躲避玩家的方式。
  - 偶遇卡会显示当前动物活动状态，让玩家知道它正在做什么。
- ✅ 不同地貌区域的主线目标开始分化：
  - 草原默认先导向迁徙动物与迁徙/水源热点。
  - 湿地默认先导向水源热点、水域动物和飞行动物。
  - 森林默认先导向林荫/倒木热点与林下生物。
  - 海岸默认先导向潮汐/礁岩热点与海岸生物。
  - 右下角主线步骤也会按地貌改文字，不再所有区域都显示同一套目标。
- ✅ 热点名称和说明也按地貌本地化：
  - 草原显示 `季节水洼 / 草食迁徙带 / 掠食高地`。
  - 湿地显示 `浅滩水眼 / 芦苇通行带 / 泥洲伏击线`。
  - 森林显示 `林下水洼 / 兽径通道 / 密林荫蔽区`。
  - 海岸显示 `潮汐水线 / 海岸通行带 / 漂上海滩`。
  - 热点卡和生态观察播报会解释该热点在当前地貌下的生态作用。
- ✅ 所有区域现在都有基础撤离口：
  - 固定生成 `调查营地` 出口，保证没有相邻通道的区域也能按 `E` 撤离并回灌报告。
  - 通道出口从第二个出口开始生成，不再覆盖基础撤离口。
  - 通道线会优先导向真正带目标区域的通道出口；如果没有通道出口，会提示先补调查来解锁连接。
- ✅ Godot 前端继续收口 2D 主线，不再沿 3D 探索资产路线返工。
- ✅ 世界地图改成 `写实地形底图优先`：
  - 移除大面积彩色生态圆形覆盖层。
  - 区域标记缩小为低干扰地图钉。
  - 只强调当前区域、建议优先区和通道目标。
- ✅ 世界 HUD 改成更直接的玩法语句：
  ...
  - `建议`
  - `进入后任务`
  - `操作顺序：选行动 -> 进入区域 -> 撤离后回灌报告`
- ✅ 增加撤离报告待回灌保护：有未回灌报告时，不能继续切行动、切建议区或进入下一片区域，必须先把探索结果写回后端生态系统。
- ✅ 区域探索报告增加 `report_id / created_at`，后端导出会保留并用它判断报告是否已经回灌，避免同一份撤离报告重复应用或误判。
- ✅ 区域探索默认 HUD 改成轻量模式：详细任务/图鉴面板默认隐藏，玩家按 `Tab` 才打开；默认画面只保留目标、当前可交互对象、小雷达和紧凑任务追踪器。
- ✅ 区域探索现场表现继续降噪：地表增加低透明自然纹理，热点标签只在当前目标/靠近时显示，水源和腐食地改为更自然的斑块，玩家角色从几何占位改为 2D 探险者剪影。
- ✅ 区域探索反馈继续游戏化：紧凑任务追踪器增加进度条，记录/采样/撤离奖励浮层压缩为短奖励提示，明确显示回灌后对世界的影响。
- ✅ 区域探索交互提示继续清晰化：靠近动物/热点/出口时，场景中直接出现 `Space 记录`、`Space 采样`、`E 撤离` 的轻量高亮；普通动物名称不再常驻显示，只在当前目标或可交互时显示。
- ✅ 世界地图待回灌状态强化为主任务态：有撤离报告未回灌时，HUD 标题切换为“撤离报告待回灌”，边框变为金色，并统一提示点击 `回灌报告`。
- ✅ 区域探索主线继续做真实感和生态行为改造：
  - 新增草原、湿地、森林、海岸四张自然纹理底图，区域内不再主要依赖大块几何色块模拟地形。
  - 区域底图改为运行时直接加载 PNG，避免首次运行因 Godot import 缓存缺失导致贴图不显示。
  - 动物移动从固定绕圈公式改为目标点驱动，每个动物会保存当前目标、停留时间和移动速度。
  - 动物行为会根据迁移、取水、觅食、躲避玩家、追猎草食群等状态改变目标和速度，减少“固定区域绕圈”的观感。
- ✅ 区域探索主线引导继续强化：
  - 黄色目标引导改成玩家身前的方向扇形、路径点和距离标签，减少不知道往哪走的问题。
  - 顶部主任务卡增加“回灌后端会改变什么”，把现场操作和后端生态系统收益连起来。
  - 右下主线进度卡显示 4 条步骤和后端影响，不再只给抽象进度。
  - 动物靠近玩家时不再频繁逃离；按住 `Space` 调查时会保持可记录距离，减少目标刚接近就跑开的体验。
- ✅ 地貌差异和后端反馈继续加强：
  - 草原、湿地、森林、海岸现在会轮播各自的现场生态提示，不再只靠同一套通用播报。
  - 完成动物记录或热点采样后，奖励浮层会说明该情报回灌后影响水源、迁徙、压迫、腐食或栖地哪类后端判断。
  - 现场事件文案强调每种地貌的玩法重点：草原看迁徙/掠食，湿地看水位/芦苇，森林看庇护/倒木，海岸看潮线/通道。
- ✅ 主线章节接入后端 `gameplay_state`：
  - 后端导出现在会生成主线章节、当前目标、推荐区域、推荐行动和主线进度。
  - 世界图优先跟随主线焦点区域和推荐行动，不再只按单区风险临时推荐。
  - 进入区域时会把主线章节和目标写进调查请求，区域内开场任务会显示这轮属于哪一章、为什么执行当前行动。
- ✅ 主线章节进度变成可见状态：
  - 后端会为每章导出 `progress_label / progress_ratio / next_unlock`。
  - 世界图 HUD 直接显示当前章节完成度和完成后会解锁的下一类主线。
  - 第一章看撤离报告回灌，第二章看修复线，第三章看通道线，第四章看安全生态区数量。
- ✅ 回灌报告会显示章节推进：
  - 撤离报告现在会记录所属主线章节和章节目标。
  - 回灌完成后，世界图会对比“报告所属章节”和“后端当前章节”。
  - 如果章节变化，会直接显示 `主线推进：上一章 -> 下一章` 和下一轮目标。
- ✅ 主线行动优先级修正：
  - 当主线章节指定焦点区域和推荐行动时，进入区域请求会以主线为准覆盖单区 `gameplay_hint`。
  - 避免世界图显示“调查线”，区域内却沿用该区域旧的“修复风险”原因。
  - 通道章节也会把主线目标区域写入进入请求，保证区域内出口目标和世界图一致。
- ✅ 区域内世界任务完成条件收紧：
  - 调查线现在必须完成 `记录 1 种动物 + 采样 1 个热点 + 情报 3 点`，不再只采样一个热点就算完成。
  - 修复线继续要求 `采样热点 + 情报 3 点`。
  - 撤离准备状态会跟世界任务完成同步，任务达成后才稳定引导玩家去出口回灌。
- ✅ 区域内主线缺口提示更明确：
  - 顶部主线提示会直接说明当前还差动物记录、热点采样、情报点数还是通道出口。
  - 右下主线进度卡会优先显示当前缺口，而不是只显示后端收益说明。
  - 主线目标首次完成时会弹出“主线目标完成”，提示立刻去出口撤离并回灌报告。
...
- ✅ [src/ui/world_payload.py](/Users/yumini/Projects/eco-world/src/ui/world_payload.py) 现在新增：
  - `frontier_directive_decisions`
  - 每套定案的 `decision_band / decision_name`
- ✅ [godot/scripts/world_map.gd](/Users/yumini/Projects/eco-world/godot/scripts/world_map.gd) 现在新增：
  - `指令定案终端`
  - 主视区显式显示 `当前定案 / 定案主轴`
- ✅ 这意味着当前 Godot 世界入口已经不只是做比选，而开始把比选结果推进成正式战区定案。
...
- ✅ [godot/scripts/world_map.gd](/Users/yumini/Projects/eco-world/godot/scripts/world_map.gd) 现在新增：
  - 独立的 `锁定推进落点`
  - 地图候选落点徽记可直接锁定
  - 右侧 `落点网络总板` 的每条候选都可直接锁定
  - 当前锁定落点会重排中心主舞台、地图徽记和总览摘要
- ✅ 这意味着当前战区已经不只是看筛选结果，而开始能把某个候选分支明确锁成当前推进目标。

### v4.0-alpha135 (2026-04-14 14:03)

- ✅ Godot 前端这轮把 `战区比较视图` 继续推进成了 `战区筛选视图`。
- ✅ [godot/scripts/world_map.gd](/Users/yumini/Projects/eco-world/godot/scripts/world_map.gd) 现在新增：
  - `综合 / 稳态 / 高繁荣 / 高风险` 四种筛选模式
  - 候选落点按当前筛选模式重新排序
  - 主舞台、战区模式条、落点比较板同步当前筛选
- ✅ 这意味着当前战区已经不只是固定看一套排序，而开始能按不同推进目标快速筛选分支。

### v4.0-alpha134 (2026-04-14 13:50)

- ✅ Godot 前端这轮把 `战区落点网络视图` 继续推进成了 `战区比较视图`。
- ✅ [godot/scripts/world_map.gd](/Users/yumini/Projects/eco-world/godot/scripts/world_map.gd) 现在新增：
  - 候选落点评分
  - 地图上的 `优选` 候选落点提示
  - 右侧 `优选推进 / 当前落点 / 高风险分支` 比较栏
  - 主舞台里的 `优选落点`
- ✅ 这意味着当前战区已经不只是并列显示多个落点，而开始直接比较哪条分支更值得推进。

### v4.0-alpha133 (2026-04-14 13:39)

- ✅ Godot 前端这轮把 `区域落点视图` 继续推进成了 `战区落点网络视图`。
- ✅ [godot/scripts/world_map.gd](/Users/yumini/Projects/eco-world/godot/scripts/world_map.gd) 现在新增：
  - 多个候选落点同时显示
  - 地图上的候选落点徽记
  - 右侧 `落点网络总板`
  - 中心主舞台同步当前落点区域
- ✅ 这意味着当前战区下不再只看一个落点，而开始能比较多个候选推进分支。

### v4.0-alpha132 (2026-04-14 13:28)

- ✅ Godot 前端这轮把 `区域推进视图` 继续推进成了 `区域落点视图`。
- ✅ [godot/scripts/world_map.gd](/Users/yumini/Projects/eco-world/godot/scripts/world_map.gd) 现在新增：
  - 当前推进阶段的落点区域摘要
  - 第一阶段/第二阶段的地图落点入口
  - 右侧 `推进落点终端`
  - 中心主舞台同步当前落点
- ✅ 这意味着地图上的阶段标记不再只是提示，而开始成为真正的推进落点入口。

### v4.0-alpha131 (2026-04-14 13:16)

- ✅ Godot 前端这轮把 `战略地图模式` 继续推进成了 `区域推进视图`。
- ✅ [godot/scripts/world_map.gd](/Users/yumini/Projects/eco-world/godot/scripts/world_map.gd) 现在新增：
  - `第一阶段 / 第二阶段` 的推进切换
  - 中心主舞台按当前阶段显示不同抬头和目标
  - 地图战区覆盖层按阶段切换强弱
  - 右侧 `战区图谱总板` 同步当前阶段
- ✅ 这意味着当前 campaign 不再只是一个静态战区模式，而开始具备真实的阶段化推进视图。
