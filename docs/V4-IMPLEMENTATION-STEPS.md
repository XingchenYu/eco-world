...
- 这层新的 runtime 条件锚点现在也已继续进入：
  ...
  - `carrion_chain`
  的摘要、区域反馈与低频重平衡
- 这层新的 `condition_anchor_pull` 现在也已继续进入：
  - 世界级长期压力聚合
...
- `regional_health_anchor` 现也开始继续进入 `lion / hyena` 的运行期体况层
- 它现在也会直接改善 apex 运行体的：
  - `health`
  - `hunger`
  - `reproduction_rate`
...
- 这些真实体况信号现在已经开始进入：
  ...
  也就是说，系统现在已经不只看锚点、偏置和热点，也开始把动物当下真实状态并入长期社群节律
- `grassland_chain` 现也开始继续读取：
  - `herd_condition_runtime`
  - `apex_condition_runtime`
  并把它们写成：
