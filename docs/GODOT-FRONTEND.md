...
- 主视区显式显示当前预案和三段路线序列
- 右侧总览显式收口三套编成预案
- 前端读取 `frontier_activation_profiles`
- 新增 `预案激活终端`
- 右侧总览可直接激活某套战区预案
- 激活后同步重排当前战区、编成、调度路线与落点
- 前端读取 `frontier_activation_feedbacks`
- 新增 `预案回路终端`
- 激活预案后会自动回灌战区筛选、推进阶段与优先落点
- 主视区和总览都会显式显示当前回路推荐
- 回路会继续重排战区图谱、候选落点和调度路线顺序
- 前端读取 `frontier_directive_profiles`
- 新增 `战区指令终端`
- 可直接切换 `突进 / 均衡 / 稳态` 三套指令
- 指令会同步改写筛选、阶段、落点和调度主轴
- 前端读取 `frontier_directive_previews`
- 新增 `指令预演终端`
- 主视区和总览会显式显示 指令一阶 / 二阶 / 回退 预演
- 前端读取 `frontier_directive_sandbox`
- 新增 `指令沙盘终端`
- 可并排比较三套战区指令的沙盘总分和主轴落点
- 前端读取 `frontier_directive_comparison`
- 新增 `指令比选终端`
- 可直接查看 优选指令 / 当前激活 / 高风险指令
- 前端读取 `frontier_directive_decisions`
- 新增 `指令定案终端`
- 主视区和总览会显式显示当前定案和定案主轴
- 前端读取 `frontier_directive_locks`
- 新增 `定案锁定终端`
- 可直接锁定当前战区定案，并同步改写当前指令、预案与编成
- 前端读取 `frontier_directive_confirmations`
- 新增 `定案确认终端`
- 主视区和总览会显式显示 当前确认 / 确认主轴 / 确认带
- 地图空间中的前线主走廊
- 地图空间中的二级分支线
- 当前前线目标与分支区域空间徽记
- 手动刷新
