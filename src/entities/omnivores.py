...
def _social_group_birth(animal: Animal, ecosystem, social_factor: float, stable_label: str, unstable_label: str):
    ...
    birth_cycle_window_pressure_bias = max(0.0, min(1.0, getattr(animal, "birth_cycle_window_pressure_bias", 0.0)))
...
class Lion(Animal):
    ...
    def _apply_world_pressure_window_bias(self):
        ...
        if bias <= 0.0:
            return
        self.health = min(getattr(self, "max_health", 100), self.health + bias * 0.07)
    ...
    def _apply_birth_memory_world_pressure_bias(self):
        ...
        if bias <= 0.0:
            return
        self.health = min(getattr(self, "max_health", 100), self.health + bias * 0.07)
        self.hunger = max(0.0, self.hunger - bias * 0.11)
        if bias >= 0.24:
            self.mate_cooldown = max(0, self.mate_cooldown - 1)
        self.reproduction_rate *= 1.0 + bias * 0.006
...
class Hyena(Animal):
    ...
    def _apply_condition_runtime(self):
        ...
        if condition <= 0.0:
            return
        self.health = min(getattr(self, "max_health", 100), self.health + condition * 0.13)
        self.hunger = max(0.0, self.hunger - condition * 0.18)
        cooldown_relief = max(1, int(condition * 2)) if condition >= 0.35 else int(condition * 2)
        self.mate_cooldown = max(0, self.mate_cooldown - cooldown_relief)
