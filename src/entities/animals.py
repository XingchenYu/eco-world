...
class Animal(Creature):
    ...
    def _give_birth(self, ecosystem):
        ...
        birth_cycle_window_pressure_bias = max(0.0, min(1.0, getattr(self, "birth_cycle_window_pressure_bias", 0.0)))
        condition_factor = max(
            ...
            min(
                ...
                + birth_cycle_window_pressure_bias * 0.08
            ),
        )
            ...
            min(
                ...
                int(
                    round(
                        ...
                        - birth_cycle_bias * 2
                        - birth_cycle_window_pressure_bias * 2
                    )
                ),
            ),
        )
...
class Antelope(Animal):
    ...
    def __init__(self, position: Tuple[int, int], gender: Gender = None):
        ...
        self.birth_cycle_window_pressure_bias = 0.0
    ...
    def execute_behavior(self, ecosystem):
        ...
        self._apply_birth_cycle_window_pressure_bias()
    ...
    def _apply_condition_phase_bias(self):
        ...
        if phase_bias <= 0.0:
            return
        self.health = min(getattr(self, "max_health", 100), self.health + phase_bias * 0.12)
        self.hunger = max(0.0, self.hunger - phase_bias * 0.18)
        if phase_bias >= 0.28:
            self.mate_cooldown = max(0, self.mate_cooldown - 1)
        self.reproduction_rate *= 1.0 + phase_bias * 0.010
    ...
    def _apply_world_pressure_window_bias(self):
        bias = max(0.0, min(1.0, self.world_pressure_window_bias))
        if bias <= 0.0:
            return
    ...
    def _apply_birth_memory_world_pressure_bias(self):
        bias = max(0.0, min(1.0, self.birth_memory_world_pressure_bias))
        if bias <= 0.0:
            return
        self.health = min(getattr(self, "max_health", 100), self.health + bias * 0.08)
        self.hunger = max(0.0, self.hunger - bias * 0.12)
    ...
    def _apply_birth_cycle_window_pressure_bias(self):
        bias = max(0.0, min(1.0, self.birth_cycle_window_pressure_bias))
        if bias <= 0.0:
            return
        self.health = min(getattr(self, "max_health", 100), self.health + bias * 0.05)
        self.hunger = max(0.0, self.hunger - bias * 0.08)
        if bias >= 0.24:
            self.mate_cooldown = max(0, self.mate_cooldown - 1)
...
class Zebra(Animal):
    ...
    def _apply_condition_phase_bias(self):
        ...
        if phase_bias <= 0.0:
            return
        self.health = min(getattr(self, "max_health", 100), self.health + phase_bias * 0.11)
        self.hunger = max(0.0, self.hunger - phase_bias * 0.16)
        if phase_bias >= 0.28:
            self.mate_cooldown = max(0, self.mate_cooldown - 1)
...
class Vulture(Animal):
    ...
    def _apply_condition_phase_bias(self):
        ...
        if phase_bias <= 0.0:
            return
        self.health = min(getattr(self, "max_health", 100), self.health + phase_bias * 0.11)
        self.hunger = max(0.0, self.hunger - phase_bias * 0.17)
        if phase_bias >= 0.28:
            self.mate_cooldown = max(0, self.mate_cooldown - 1)
        self.reproduction_rate *= 1.0 + phase_bias * 0.010
