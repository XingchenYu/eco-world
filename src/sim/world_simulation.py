...
class WorldSimulation:
    ...
    def _build_combined_pressures(
        ...
        prosperity_pressure = (
            ...
            + float(grassland_chain.trophic_scores.get("birth_cycle_window_pressure_memory_pull", 0.0))
            ...
            + float(grassland_chain.trophic_scores.get("runtime_apex_condition_pull", 0.0)) * 0.7
            ...
            + float(carrion_chain.resource_scores.get("birth_cycle_window_pressure_memory_pull", 0.0))
        ...
        collapse_pressure = (
            ...
            + float(grassland_chain.trophic_scores.get("runtime_apex_condition_pull", 0.0)) * 0.28
            ...
            + float(grassland_chain.trophic_scores.get("birth_cycle_window_pressure_memory_pull", 0.0)) * 0.08
            ...
            + float(carrion_chain.resource_scores.get("birth_cycle_window_pressure_memory_pull", 0.0)) * 0.08
        ...
        runtime_resource_pressure = (
            ...
            + float(territory.runtime_signals.get("herd_anchor_prosperity_runtime", 0.0))
            + float(territory.runtime_signals.get("aerial_anchor_prosperity_runtime", 0.0))
            ...
            + (0.14 if "world_pressure_window_memory" in social_trends.cycle_signals else 0.0)
        )
    ...
    def _build_runtime_territory_state(self, simulation: RegionSimulation) -> Dict[str, float]:
        ...
        state = {
            ...
            "apex_world_pressure_runtime": 0.0,
            "herd_world_pressure_runtime": 0.0,
            "aerial_world_pressure_runtime": 0.0,
            ...
            "apex_birth_memory_runtime": 0.0,
            "herd_birth_memory_runtime": 0.0,
            "aerial_birth_memory_runtime": 0.0,
        ...
        if lions:
            ...
            state["apex_condition_phase_bias_runtime"] = max(
                [getattr(animal, "condition_phase_bias", 0.0) for animal in lions] or [0.0]
            )
            ...
            state["apex_condition_anchor_runtime"] = min(
                1.0,
                state["apex_condition_runtime"] * 0.62
            ...
            state["apex_world_pressure_runtime"] = max(
                [getattr(animal, "world_pressure_bias", 0.0) for animal in lions] or [0.0]
            )
            state["apex_world_pressure_window_runtime"] = max(
                [getattr(animal, "world_pressure_window_bias", 0.0) for animal in lions] or [0.0]
            )
            state["apex_birth_memory_runtime"] = max(
                [getattr(animal, "birth_memory_bias", 0.0) for animal in lions] or [0.0]
            )
            ...
            state["apex_birth_cycle_window_runtime"] = min(
                1.0,
        ...
        if hyenas:
            ...
            state["apex_condition_phase_bias_runtime"] = max(
                state["apex_condition_phase_bias_runtime"],
                max([getattr(animal, "condition_phase_bias", 0.0) for animal in hyenas] or [0.0]),
            )
            ...
            state["apex_condition_anchor_runtime"] = min(
                1.0,
                max(
                    state["apex_condition_anchor_runtime"],
                    state["apex_condition_runtime"] * 0.62
                    + state["apex_regional_health_runtime"] * 0.22
                    + state["apex_anchor_prosperity_runtime"] * 0.10
                    + float(region_resource_state.get("carcass_availability", 0.0)) * 0.06,
            ...
            state["apex_world_pressure_runtime"] = max(
                state["apex_world_pressure_runtime"],
                max([getattr(animal, "world_pressure_bias", 0.0) for animal in hyenas] or [0.0]),
            )
            state["apex_world_pressure_window_runtime"] = max(
                state["apex_world_pressure_window_runtime"],
                max([getattr(animal, "world_pressure_window_bias", 0.0) for animal in hyenas] or [0.0]),
            )
            state["apex_birth_memory_runtime"] = max(
                state["apex_birth_memory_runtime"],
                max([getattr(animal, "birth_memory_bias", 0.0) for animal in hyenas] or [0.0]),
            )
            state["apex_birth_memory_world_pressure_runtime"] = max(
            ...
            state["apex_birth_cycle_window_runtime"] = max(
                state["apex_birth_cycle_window_runtime"],
                min(
                    1.0,
                    state["apex_birth_cycle_runtime"] * 0.64
                    + birth_cycle_window_memory * 0.28
        ...
        if antelopes or zebras:
            ...
            state["herd_condition_phase_bias_runtime"] = max(
                [getattr(animal, "condition_phase_bias", 0.0) for animal in antelopes + zebras] or [0.0]
            )
            ...
            state["herd_condition_anchor_runtime"] = min(
                1.0,
                state["herd_condition_runtime"] * 0.60
                + state["herd_regional_health_runtime"] * 0.20
            ...
            state["herd_world_pressure_runtime"] = max(
                [getattr(animal, "world_pressure_bias", 0.0) for animal in antelopes + zebras] or [0.0]
            )
            state["herd_world_pressure_window_runtime"] = max(
                [getattr(animal, "world_pressure_window_bias", 0.0) for animal in antelopes + zebras] or [0.0]
            )
            state["herd_birth_memory_runtime"] = max(
                [getattr(animal, "birth_memory_bias", 0.0) for animal in antelopes + zebras] or [0.0]
            )
            ...
            state["herd_birth_cycle_window_runtime"] = min(
                1.0,
        ...
        if vultures:
            ...
            state["aerial_condition_phase_bias_runtime"] = max(
                [getattr(animal, "condition_phase_bias", 0.0) for animal in vultures] or [0.0]
            )
            ...
            state["aerial_condition_anchor_runtime"] = min(
                1.0,
            ...
            state["aerial_world_pressure_runtime"] = max(
                [getattr(animal, "world_pressure_bias", 0.0) for animal in vultures] or [0.0]
            )
            state["aerial_world_pressure_window_runtime"] = max(
                [getattr(animal, "world_pressure_window_bias", 0.0) for animal in vultures] or [0.0]
            )
            state["aerial_birth_memory_runtime"] = max(
                [getattr(animal, "birth_memory_bias", 0.0) for animal in vultures] or [0.0]
            )
            ...
            state["aerial_birth_cycle_window_runtime"] = min(
                1.0,
