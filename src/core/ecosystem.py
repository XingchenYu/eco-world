...
from ..entities.omnivores import (
    Bear, Beaver, Hippopotamus, Elephant, WhiteRhino, Giraffe, WildBoar, Badger, RaccoonDog, Skunk, 
...
class Ecosystem:
    ...
    DEFAULT_INITIAL_POPULATION = {
        ...
        "bear": 40, "beaver": 30, "hippopotamus": 16, "elephant": 12, "white_rhino": 8, "giraffe": 10, "wild_boar": 100, "badger": 80, "raccoon_dog": 60, "skunk": 80, "opossum": 100, "coati": 60, "armadillo": 60,
    ...
    LAND_ANIMAL_SPECIES = [
        ...
        "squirrel", "hedgehog", "bat", "raccoon",
        "bear", "beaver", "hippopotamus", "elephant", "white_rhino", "giraffe", "wild_boar", "badger", "raccoon_dog",
    ...
    def _init_population(self):
        ...
        for _ in range(initial.get("beaver", 3)):
            pos = self._random_water_adjacent_position() or self._random_land_position()
            if pos:
                self.animals.append(Beaver(pos))

        for _ in range(initial.get("hippopotamus", 2)):
            pos = self._random_water_adjacent_position() or self._random_water_position_for_body_type({"river_channel", "lake_shallow"})
            if pos:
        ...
        for _ in range(initial.get("white_rhino", 1)):
            pos = self._random_land_position()
            if pos:
                self.animals.append(WhiteRhino(pos))

        for _ in range(initial.get("giraffe", 2)):
            pos = self._random_land_position()
            if pos:
                self.animals.append(Giraffe(pos))
    ...
    def spawn_animal(self, species: str, position: Tuple[int, int], gender: Gender = None, is_offspring: bool = False, source: str = "natural"):
        ...
        animal_classes = {
            ...
            # 新增杂食动物
            "bear": Bear, "beaver": Beaver, "hippopotamus": Hippopotamus, "elephant": Elephant, "white_rhino": WhiteRhino, "giraffe": Giraffe, "wild_boar": WildBoar, "badger": Badger,
