...
def build_region_social_trend_summary(
        ...
        "grassland_bust_phase": round(
            max(
                ...
                min(
                    ...
                    - herd_resource_anchor_runtime * 0.03
                    - herd_world_pressure_runtime * 0.02
        ...
        "vulture_hotspot_memory": round(
            max(
                ...
                min(
                    ...
                    + aerial_world_pressure_runtime * 0.04
        ...
        "vulture_carrion_memory": round(
            max(
                ...
                min(
                    ...
                    + apex_world_pressure_runtime * 0.03
                    + apex_regional_health_anchor_runtime * 0.03
                    + apex_regional_bias_runtime * 0.03
                    + carcass_anchor * 0.05
                    + regional_prosperity * 0.03
    ...
    if herd_resource_anchor_runtime >= 0.42:
        cycle_signals.append("herd_resource_anchor_runtime")
    if herd_anchor_prosperity_runtime >= 0.40:
        cycle_signals.append("herd_anchor_prosperity_runtime")
    ...
    if herd_condition_phase_runtime >= 0.30:
    ...
    if aerial_resource_anchor_runtime >= 0.38:
        cycle_signals.append("aerial_resource_anchor_runtime")
    if aerial_anchor_prosperity_runtime >= 0.36:
        cycle_signals.append("aerial_anchor_prosperity_runtime")
    if apex_anchor_prosperity_runtime >= 0.34:
        cycle_signals.append("apex_anchor_prosperity_runtime")
    ...
    if aerial_condition_phase_runtime >= 0.28:
    ...
    if apex_condition_phase_runtime >= 0.26:
...
def apply_region_social_trend_feedback(
    ...
    _adjust(region.resource_state, "surface_water", scores.get("birth_cycle_window_memory_strength", 0.0) * 0.06, feedback_scale)
    _adjust(region.resource_state, "carcass_availability", scores.get("birth_cycle_window_memory_strength", 0.0) * 0.05, feedback_scale)
    _adjust(region.resource_state, "surface_water", scores.get("birth_cycle_window_pressure_memory", 0.0) * 0.05, feedback_scale)
    _adjust(region.resource_state, "carcass_availability", scores.get("birth_cycle_window_pressure_memory", 0.0) * 0.05, feedback_scale)
    _adjust(region.resource_state, "surface_water", social_trends.prosperity_scores.get("grassland_prosperity_phase", 0.0) * 0.08, feedback_scale)
    _adjust(region.resource_state, "surface_water", -social_trends.prosperity_scores.get("grassland_collapse_phase", 0.0) * 0.05, feedback_scale)
    _adjust(region.resource_state, "carcass_availability", social_trends.prosperity_scores.get("grassland_prosperity_phase", 0.0) * 0.06, feedback_scale)
    _adjust(region.resource_state, "carcass_availability", -social_trends.prosperity_scores.get("grassland_collapse_phase", 0.0) * 0.04, feedback_scale)
    ...
    _adjust(region.health_state, "resilience", scores.get("birth_cycle_window_memory_strength", 0.0) * 0.08, feedback_scale)
    _adjust(region.health_state, "resilience", scores.get("birth_cycle_window_pressure_memory", 0.0) * 0.07, feedback_scale)
