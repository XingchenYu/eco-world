...
def build_region_grassland_chain_summary(
    ...
    if territory_summary is not None:
        ...
        if herd_birth_cycle_runtime > 0.0:
            add_score("runtime_herd_birth_cycle_pull", min(0.18, herd_birth_cycle_runtime * 0.12), "多周期 herd 产仔节律正在继续抬升草原走廊的恢复与延续强度。")
            add_layer_bias("herd_layer", herd_birth_cycle_runtime * 0.06)
            add_layer_bias("social_layer", herd_birth_cycle_runtime * 0.03)
        ...
        if herd_world_pressure_runtime > 0.0:
            add_score("runtime_herd_world_pressure_pull", min(0.18, herd_world_pressure_runtime * 0.12), "世界级长期压力回灌成的 herd 偏置，正在继续抬升草原走廊的跨周期稳定度。")
            add_layer_bias("herd_layer", herd_world_pressure_runtime * 0.06)
            add_layer_bias("social_layer", herd_world_pressure_runtime * 0.03)
        ...
        if herd_birth_cycle_window_pressure_runtime > 0.0:
            add_score("runtime_herd_birth_cycle_window_pressure_pull", min(0.18, herd_birth_cycle_window_pressure_runtime * 0.12), "繁殖窗口强度沉淀成的 herd 运行期压力偏置，正在继续抬升草原走廊的恢复强度。")
            add_layer_bias("herd_layer", herd_birth_cycle_window_pressure_runtime * 0.06)
            add_layer_bias("social_layer", herd_birth_cycle_window_pressure_runtime * 0.03)
        ...
        if apex_birth_cycle_runtime > 0.0:
            add_score("runtime_apex_birth_cycle_pull", min(0.16, apex_birth_cycle_runtime * 0.11), "多周期 apex 产仔节律正在继续抬升顶层热点与延续前线。")
            add_layer_bias("predator_layer", apex_birth_cycle_runtime * 0.05)
            add_layer_bias("social_layer", apex_birth_cycle_runtime * 0.03)
        ...
        if apex_world_pressure_runtime > 0.0:
            add_score("runtime_apex_world_pressure_pull", min(0.16, apex_world_pressure_runtime * 0.11), "世界级长期压力回灌成的 apex 偏置，正在继续抬升顶层热点和捕食前线。")
            add_layer_bias("predator_layer", apex_world_pressure_runtime * 0.05)
            add_layer_bias("social_layer", apex_world_pressure_runtime * 0.03)
        ...
        if apex_birth_cycle_window_pressure_runtime > 0.0:
            add_score("runtime_apex_birth_cycle_window_pressure_pull", min(0.16, apex_birth_cycle_window_pressure_runtime * 0.11), "繁殖窗口强度沉淀成的 apex 运行期压力偏置，正在继续抬升顶层热点恢复前线。")
            add_layer_bias("predator_layer", apex_birth_cycle_window_pressure_runtime * 0.05)
            add_layer_bias("social_layer", apex_birth_cycle_window_pressure_runtime * 0.03)
    ...
    if social_trend_summary is not None:
        ...
        if birth_cycle_window_memory_strength > 0.0:
            add_score(
                ...
                min(0.22, birth_cycle_window_memory_strength * 0.20),
                "繁殖周期窗口的长期强度记忆正在把草原 herd 与社群层重新拉回更稳定的繁殖通道。",
            )
...
def apply_region_grassland_chain_feedback(
    ...
    _adjust(region.resource_state, "surface_water", scores.get("runtime_herd_birth_cycle_pull", 0.0) * 0.10 * herd_bias, feedback_scale)
    _adjust(region.resource_state, "surface_water", scores.get("runtime_herd_birth_cycle_window_pull", 0.0) * 0.10 * herd_bias, feedback_scale)
    _adjust(region.resource_state, "surface_water", scores.get("runtime_herd_birth_cycle_window_pressure_pull", 0.0) * 0.10 * herd_bias, feedback_scale)
    _adjust(region.resource_state, "surface_water", scores.get("birth_cycle_window_memory_strength_pull", 0.0) * 0.10 * herd_bias, feedback_scale)
    _adjust(region.resource_state, "surface_water", scores.get("birth_cycle_window_pressure_memory_pull", 0.0) * 0.10 * herd_bias, feedback_scale)
    ...
    _adjust(region.hazard_state, "predation_pressure", scores.get("runtime_apex_birth_cycle_pull", 0.0) * 0.10 * predator_bias, feedback_scale)
    _adjust(region.hazard_state, "predation_pressure", scores.get("runtime_apex_birth_cycle_window_pull", 0.0) * 0.10 * predator_bias, feedback_scale)
    _adjust(region.hazard_state, "predation_pressure", scores.get("runtime_apex_birth_cycle_window_pressure_pull", 0.0) * 0.10 * predator_bias, feedback_scale)
    ...
    _adjust(region.health_state, "resilience", scores.get("runtime_herd_birth_cycle_pull", 0.0) * 0.08 * prosperity_bias, feedback_scale)
    _adjust(region.health_state, "resilience", scores.get("runtime_herd_birth_cycle_window_pull", 0.0) * 0.08 * prosperity_bias, feedback_scale)
    _adjust(region.health_state, "resilience", scores.get("runtime_herd_birth_cycle_window_pressure_pull", 0.0) * 0.08 * prosperity_bias, feedback_scale)
    ...
    _adjust(region.health_state, "resilience", scores.get("birth_cycle_window_pressure_memory_pull", 0.0) * 0.08 * prosperity_bias, feedback_scale)
    ...
    _adjust(region.health_state, "resilience", scores.get("runtime_apex_birth_cycle_window_pull", 0.0) * 0.08 * prosperity_bias, feedback_scale)
    _adjust(region.health_state, "resilience", scores.get("runtime_apex_birth_cycle_window_pressure_pull", 0.0) * 0.08 * prosperity_bias, feedback_scale)
...
def apply_region_grassland_chain_rebalancing(
    ...
    if scores.get("runtime_herd_birth_pull", 0.0) >= 0.06 and antelope_count < 19:
        species_pool["antelope"] = species_pool.get("antelope", 0) + 1
        adjustments.append(
            {
                "source_species": "runtime_birth",
                "target_species": "antelope",
                "layer_group": "herd_layer",
                "effect": "runtime_herd_birth_support",
                "new_target_count": species_pool["antelope"],
            }
        )
    if scores.get("runtime_herd_birth_memory_pull", 0.0) >= 0.05 and antelope_count < 19:
        species_pool["antelope"] = species_pool.get("antelope", 0) + 1
        adjustments.append(
            {
                ...
                "target_species": "antelope",
                "layer_group": "herd_layer",
                "effect": "runtime_herd_birth_memory_support",
                "new_target_count": species_pool["antelope"],
            }
        )
    ...
    if scores.get("runtime_herd_birth_pull", 0.0) >= 0.06 and zebra_count < 17:
        species_pool["zebra"] = species_pool.get("zebra", 0) + 1
        adjustments.append(
            {
                "source_species": "runtime_birth",
    ...
    if scores.get("runtime_herd_birth_memory_pull", 0.0) >= 0.05 and zebra_count < 17:
        species_pool["zebra"] = species_pool.get("zebra", 0) + 1
        adjustments.append(
            {
                "source_species": "runtime_birth_memory",
    ...
    if scores.get("runtime_apex_birth_pull", 0.0) >= 0.05 and lion_count < 8:
        species_pool["lion"] = species_pool.get("lion", 0) + 1
        adjustments.append(
            {
                "source_species": "runtime_birth",
    ...
    if scores.get("runtime_apex_birth_pull", 0.0) >= 0.05 and hyena_count < 8:
        species_pool["hyena"] = species_pool.get("hyena", 0) + 1
        adjustments.append(
            {
                "source_species": "runtime_birth",
    ...
    if scores.get("runtime_herd_birth_cycle_pull", 0.0) >= 0.04 and zebra_count < 18:
        species_pool["zebra"] = species_pool.get("zebra", 0) + 1
        adjustments.append(
            {
                "source_species": "birth_cycle",
                "target_species": "zebra",
                "layer_group": "herd_layer",
                "effect": "birth_cycle_herd_support",
                "new_target_count": species_pool["zebra"],
            }
        )
    ...
    if scores.get("runtime_herd_birth_cycle_window_pull", 0.0) >= 0.04 and zebra_count < 18:
        species_pool["zebra"] = species_pool.get("zebra", 0) + 1
        adjustments.append(
            {
                "source_species": "birth_cycle_window_runtime",
                "target_species": "zebra",
                "layer_group": "herd_layer",
    ...
    if scores.get("runtime_herd_birth_cycle_window_pressure_pull", 0.0) >= 0.04 and antelope_count < 21:
        species_pool["antelope"] = species_pool.get("antelope", 0) + 1
        adjustments.append(
            {
                "source_species": "birth_cycle_window_pressure",
                "target_species": "antelope",
                "layer_group": "herd_layer",
    ...
    if scores.get("birth_cycle_window_memory_strength_pull", 0.0) >= 0.04 and antelope_count < 21:
        species_pool["antelope"] = species_pool.get("antelope", 0) + 1
        adjustments.append(
            {
                ...
                "target_species": "antelope",
                "layer_group": "herd_layer",
                "effect": "birth_cycle_window_memory_herd_support",
                "new_target_count": species_pool["antelope"],
            }
        )
    if scores.get("birth_cycle_window_pressure_memory_pull", 0.0) >= 0.04 and zebra_count < 18:
        species_pool["zebra"] = species_pool.get("zebra", 0) + 1
        adjustments.append(
            {
                "source_species": "birth_cycle_window_pressure_memory",
                "target_species": "zebra",
                "layer_group": "herd_layer",
    ...
    if scores.get("runtime_apex_birth_cycle_pull", 0.0) >= 0.04 and hyena_count < 8:
        species_pool["hyena"] = species_pool.get("hyena", 0) + 1
        adjustments.append(
            {
                "source_species": "birth_cycle",
    ...
    if scores.get("runtime_apex_birth_cycle_window_pull", 0.0) >= 0.04 and hyena_count < 8:
        species_pool["hyena"] = species_pool.get("hyena", 0) + 1
        adjustments.append(
            {
                "source_species": "birth_cycle_window_runtime",
    ...
    if scores.get("runtime_apex_birth_cycle_window_pressure_pull", 0.0) >= 0.04 and lion_count < 8:
        species_pool["lion"] = species_pool.get("lion", 0) + 1
        adjustments.append(
            {
                "source_species": "birth_cycle_window_pressure",
    ...
    if scores.get("birth_cycle_window_memory_strength_pull", 0.0) >= 0.04 and lion_count < 8:
        species_pool["lion"] = species_pool.get("lion", 0) + 1
        adjustments.append(
            {
                "source_species": "birth_cycle_window_memory_strength",
                "target_species": "lion",
                "layer_group": "predator_layer",
                "effect": "birth_cycle_window_memory_apex_support",
                "new_target_count": species_pool["lion"],
            }
        )
    if scores.get("birth_cycle_window_pressure_memory_pull", 0.0) >= 0.04 and hyena_count < 8:
        species_pool["hyena"] = species_pool.get("hyena", 0) + 1
        adjustments.append(
            {
                "source_species": "birth_cycle_window_pressure_memory",
