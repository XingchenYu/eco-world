...
def build_region_carrion_chain_summary(
    ...
    if territory_summary is not None:
        ...
        if aerial_birth_cycle_runtime > 0.0:
            add_score("runtime_aerial_birth_cycle_pull", min(0.18, aerial_birth_cycle_runtime * 0.12), "多周期空中产仔节律正在继续抬升秃鹫尸体通道的恢复与延续强度。")
            add_layer_bias("aerial_scavenge_layer", aerial_birth_cycle_runtime * 0.06)
            add_layer_bias("scavenge_layer", aerial_birth_cycle_runtime * 0.03)
        ...
        if aerial_birth_cycle_window_runtime > 0.0:
        ...
        if apex_birth_cycle_runtime > 0.0:
            add_score("runtime_apex_birth_cycle_pull", min(0.16, apex_birth_cycle_runtime * 0.11), "多周期 apex 产仔节律正在继续抬升击杀走廊与残食前线。")
            add_layer_bias("kill_layer", apex_birth_cycle_runtime * 0.05)
            add_layer_bias("scavenge_layer", apex_birth_cycle_runtime * 0.03)
...
def apply_region_carrion_chain_feedback(
    ...
    _adjust(region.resource_state, "carcass_availability", scores.get("runtime_aerial_birth_cycle_window_pressure_pull", 0.0) * 0.10 * aerial_bias, feedback_scale)
    ...
    _adjust(region.resource_state, "carcass_availability", scores.get("birth_cycle_window_pressure_memory_pull", 0.0) * 0.10 * aerial_bias, feedback_scale)
    ...
    _adjust(region.hazard_state, "predation_pressure", scores.get("runtime_apex_birth_cycle_pull", 0.0) * 0.10 * collapse_bias * kill_bias, feedback_scale)
    _adjust(region.hazard_state, "predation_pressure", scores.get("runtime_apex_birth_cycle_window_pull", 0.0) * 0.10 * collapse_bias * kill_bias, feedback_scale)
    _adjust(region.hazard_state, "predation_pressure", scores.get("runtime_apex_birth_cycle_window_pressure_pull", 0.0) * 0.10 * collapse_bias * kill_bias, feedback_scale)
    ...
    _adjust(region.health_state, "resilience", scores.get("runtime_aerial_birth_cycle_window_pressure_pull", 0.0) * 0.08 * prosperity_bias, feedback_scale)
...
def apply_region_carrion_chain_rebalancing(
    ...
    if scores.get("runtime_aerial_birth_pull", 0.0) >= 0.06 and vulture_count < 11:
        species_pool["vulture"] = species_pool.get("vulture", 0) + 1
        adjustments.append(
            {
                ...
                "target_species": "vulture",
                "layer_group": "aerial_scavenge_layer",
                "effect": "runtime_aerial_birth_support",
                "new_target_count": species_pool["vulture"],
            }
        )
    ...
    if scores.get("runtime_aerial_world_pressure_pull", 0.0) >= 0.05 and vulture_count < 12:
        species_pool["vulture"] = species_pool.get("vulture", 0) + 1
        adjustments.append(
            {
    ...
    if scores.get("runtime_aerial_birth_cycle_pull", 0.0) >= 0.04 and vulture_count < 13:
        species_pool["vulture"] = species_pool.get("vulture", 0) + 1
        adjustments.append(
            {
                ...
                "target_species": "vulture",
                "layer_group": "aerial_scavenge_layer",
                "effect": "birth_cycle_aerial_support",
                "new_target_count": species_pool["vulture"],
            }
        )
    ...
    if scores.get("runtime_aerial_birth_cycle_window_pull", 0.0) >= 0.04 and vulture_count < 13:
        species_pool["vulture"] = species_pool.get("vulture", 0) + 1
        adjustments.append(
            {
    ...
    if scores.get("runtime_aerial_birth_cycle_window_pressure_pull", 0.0) >= 0.04 and vulture_count < 13:
        species_pool["vulture"] = species_pool.get("vulture", 0) + 1
        adjustments.append(
            {
    ...
    if scores.get("birth_cycle_window_pressure_memory_pull", 0.0) >= 0.04 and vulture_count < 13:
        species_pool["vulture"] = species_pool.get("vulture", 0) + 1
        adjustments.append(
            {
    ...
    if scores.get("runtime_apex_birth_cycle_window_pressure_pull", 0.0) >= 0.04 and lion_count < 9:
        species_pool["lion"] = species_pool.get("lion", 0) + 1
        adjustments.append(
            {
                "source_species": "birth_cycle_window_pressure",
